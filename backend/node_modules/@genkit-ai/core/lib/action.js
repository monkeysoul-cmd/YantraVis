...
__export(action_exports, {
  ...
  bidiAction: () => bidiAction,
  defineAction: () => defineAction,
  defineActionAsync: () => defineActionAsync,
  defineBidiAction: () => defineBidiAction,
...
function action(config, fn) {
  ...
  actionFn.run = async (input, options) => {
        ...
        options = {
          ...
          inputStream: (async function* () {
            for await (const item of inputStream) {
              yield (0, import_schema.parseSchema)(item, {
                schema: config.inputSchema,
                jsonSchema: config.inputJsonSchema
              });
            }
    ...
    if (config.initSchema || config.initJsonSchema) {
      const validatedInit = (0, import_schema.parseSchema)(options?.init, {
...
function bidiAction(config, fn) {
  ...
  act.streamBidi = (init, opts) => {
    ...
    if (!stream) {
      channel = new import_async.Channel();
      stream = channel;
    }
